import {
  ...
  Box, Typography, Select, MenuItem, TextField, CircularProgress, IconButton,
...
export const LevelExpenseMappingDialog = ({ open, onClose }: Props) => {
  ...
  // Expense types already mapped for the selected level (exclude current row when editing)
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      expenseId: Number(form.expenseId),
      fromRange: Number(form.fromRange),
      toRange: Number(form.toRange),
    };
    if (editingId !== null) {
  ...
  const handleStartEdit = (row: ExpenseLevelMap) => {
    ...
    setForm({
      ...
      expenseId: String(matchedExpense?.id ?? ''),
      fromRange: String(row.fromRange),
      toRange: String(row.toRange),
    });
    setErrors({});
  };
  ...
  const handleConfirmDelete = () => {
    ...
    deleteMap(deleteTargetId, { onSuccess: () => setDeleteTargetId(null) });
  };
  ...
  return (
    <>
      <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth slotProps={{ paper: { sx: { borderRadius: 3 } } }}>
        <DialogTitle sx={{ fontWeight: 700 }}>Level Expense Mapping</DialogTitle>
      ...
      <Dialog open={deleteTargetId !== null} onClose={() => setDeleteTargetId(null)} maxWidth="xs" fullWidth
        ...
        <DialogContent>
          <Typography>Are you sure you want to delete this mapping? This cannot be undone.</Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
